...
import { DollarSign, Gavel, TrendingUp, Users, Package, CreditCard } from 'lucide-react'
import { StatsCard } from '@/components/dashboard/stats-card'
import { RevenueChart } from '@/components/dashboard/revenue-chart'
import { RecentActivity } from '@/components/dashboard/recent-activity'
import { LeilaoCard } from '@/components/leilao/leilao-card'
import { useAppStore } from '@/lib/store'
import { mockDashboardStats, mockLeiloes } from '@/lib/mock-data'

export default function DashboardPage() {
  const { currentUser } = useAppStore()
  const leiloesAtivos = mockLeiloes.filter(l => l.status === 'ao_vivo' || l.status === 'agendado').slice(0, 2)
